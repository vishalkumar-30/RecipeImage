...
def mask_from_eos(ids, eos_value, mult_before=True):
    mask = torch.ones(ids.size(), dtype=torch.bool, device=device)  # Changed to torch.bool
    mask_aux = torch.ones(ids.size(0), dtype=torch.bool, device=device)  # Changed to torch.bool

    for idx in range(ids.size(1)):
        if idx == 0:
            continue
        if mult_before:
            mask[:, idx] = mask[:, idx] & mask_aux  # Use logical AND for boolean operations
            mask_aux = mask_aux & (ids[:, idx] != eos_value)  # Logical AND for updating mask_aux
        else:
